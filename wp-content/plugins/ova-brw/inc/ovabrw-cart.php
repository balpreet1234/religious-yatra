...
if ( ! function_exists( 'ovabrw_validation_booking_form' ) ) {
    function ovabrw_validation_booking_form( $passed, $product_id, $quantity ) {
        ...
        $max_total_guest = intval( get_post_meta( $product_id, 'ovabrw_max_total_guest', true ) );

        if ( $max_total_guest && $max_total_guest < ( $ovabrw_adults + $ovabrw_childrens + $ovabrw_babies ) ) {
            wc_clear_notices();
            echo wc_add_notice( sprintf( esc_html__( 'Maximum total number of guests: %s', 'ova-brw' ), $max_total_guest ), 'error');
            return false;
        }

        // Min total number of guest
        $min_total_guest = intval( get_post_meta( $product_id, 'ovabrw_min_total_guest', true ) );

        if ( $min_total_guest && $min_total_guest > ( $ovabrw_adults + $ovabrw_childrens + $ovabrw_babies ) ) {
            wc_clear_notices();
            echo wc_add_notice( sprintf( esc_html__( 'Minimum total number of guests: %s', 'ova-brw' ), $min_total_guest ), 'error');
            return false;
        }
