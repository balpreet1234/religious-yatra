...
function ovabrw_create_order_show_time() {
	...
	if ( $duration ) {
		...
		if ( $check_in ) {
			$check_out = apply_filters( 'ovabrw_calculate_checkout_by_days', $check_in + absint( $number_days )*24*60*60, $check_in, $number_days );
...
function ovabrw_calculate_total() {
	...
	// Max total number of guests
	$max_guest = intval( get_post_meta( $product_id, 'ovabrw_max_total_guest', true ) );

	if ( $max_guest && $max_guest < ( $adults + $childrens + $babies ) ) {
		$data_total['error'] = sprintf( esc_html__( 'Maximum total number of guests: %s', 'ova-brw' ), $max_guest );

    	echo json_encode($data_total);
    	wp_die();
	}

	// Min total number of guests
	$min_guest = intval( get_post_meta( $product_id, 'ovabrw_min_total_guest', true ) );

	if ( $min_guest && $min_guest > ( $adults + $childrens + $babies ) ) {
		$data_total['error'] = sprintf( esc_html__( 'Minimum total number of guests: %s', 'ova-brw' ), $min_guest );
...
function ovabrw_verify_reCAPTCHA() {
	...
	if ( ! $token ) {
		...
		$url 	= curl_init('https://www.google.com/recaptcha/api/siteverify');
		$data 	= [
		    'secret' 	=> $secret_key,
		    'response' 	=> $token,
		    'remoteip' 	=> $remote_ip
